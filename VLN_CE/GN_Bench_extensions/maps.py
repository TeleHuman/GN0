...
from GN_Bench.tasks.vln.vln import VLNEpisode
from GN_Bench.utils.visualizations import maps as GN_Bench_maps

cv2 = try_cv2_import()

AGENT_SPRITE = GN_Bench_maps.AGENT_SPRITE
...
def get_top_down_map(sim, map_resolution, meters_per_pixel):
    ...
    td_map = GN_Bench_maps.get_topdown_map(
        sim.pathfinder,
        base_height,
        map_resolution,
        False,
...
def colorize_topdown_map(
    ...
    """Same as `maps.colorize_topdown_map` in GN_Bench-Lab, but with different
    colors.
    """
...
def static_to_grid(
    ...
    (coordinate_max, coordinate_max). Same as the GN_Bench-Lab maps.to_grid
...
def draw_straight_shortest_path_points(
    ...
    """
    shortest_path_points = [
        GN_Bench_maps.to_grid(p[2], p[0], img.shape[0:2], sim)[::-1]
        for p in shortest_path_points
    ]

    GN_Bench_maps.draw_path(
        img,
...
def draw_mp3d_nodes(
    ...
    for node in graph:
        ...
        if abs(pos[1] - starting_height) < 1.0:
            r_x, r_y = GN_Bench_maps.to_grid(pos[2], pos[0], img.shape[0:2], sim)
