...
from GN_Bench.tasks.nav.nav import DistanceToGoal, Success
from GN_Bench.tasks.utils import cartesian_to_polar
from GN_Bench.utils.geometry_utils import quaternion_rotate_vector
from GN_Bench.utils.visualizations import fog_of_war
from GN_Bench.utils.visualizations import maps as GN_Bench_maps
from numpy import ndarray

from VLN_CE.GN_Bench_extensions import maps
from VLN_CE.GN_Bench_extensions.task import GN_MatrixV0
...
class NDTW(Measure):
    ...
    def __init__(self, *args: Any, sim: Simulator, config: Config, **kwargs: Any):
        ...
        if "{role}" in config.GT_PATH:
            self.gt_json = {}
            for role in GN_MatrixV0.annotation_roles:
                with gzip.open(
...
class TopDownMapVLNCE(Measure):
    ...
    def get_original_map(self) -> ndarray:
        top_down_map = GN_Bench_maps.get_topdown_map_from_sim(
            self._sim,
    ...
    def reset_metric(self, *args: Any, episode: Episode, **kwargs: Any) -> None:
        self._scene_id = episode.scene_id.split("/")[-1]
        self._step_count = 0
        self._metric = None
        self._meters_per_pixel = GN_Bench_maps.calculate_meters_per_pixel(
            self._map_resolution, self._sim
        )
        self._top_down_map = self.get_original_map()
        agent_position = self._sim.get_agent_state().position
        scene_id = episode.scene_id.split("/")[-1].split(".")[0]
        a_x, a_y = GN_Bench_maps.to_grid(
            agent_position[2],
            agent_position[0],
        ...
        if self._config.FOG_OF_WAR.DRAW:
            self._fog_of_war_mask = fog_of_war.reveal_fog_of_war(
                ...
                / GN_Bench_maps.calculate_meters_per_pixel(
        ...
        self.s_x, self.s_y = GN_Bench_maps.to_grid(
            nn_position[2],
            nn_position[0],
            self._top_down_map.shape[0:2],
            self._sim,
